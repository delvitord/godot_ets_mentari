...
func _draw_woll():
	...
	var trWoll = PoolVector2Array()
	var rWoll = PoolVector2Array()
	trWoll = Trans.translate2(50, 0)
	var woll2 = Trans.transformPoints2(bolawoll, trWoll)
	rWoll.append_array(woll2)
	put_pixel_all(rWoll,Color.darkred)
	
	
func _draw_keranjang():
	var krj_bawah = PoolVector2Array()
	var px = Primitif.convert_to_kartesian(0, 0,100,-100, margin_left, margin_top)
	var keranjang = Dasar.ellipse(px[0]-tpx, px[1]-(tpy-150), 100, 20)
	...
	keranjang = Dasar.ellipse(px[0]-tpx, px[1]-(tpy), 100, 20)
	var trss = Trans.scale2(0.8, 0.5, Vector2(px[0]+tpx, px[1]-(tpy-120)))
	krj_bawah = Trans.transformPoints2(keranjang, trss)
	resk.append_array(krj_bawah)
	put_pixel_all(resk,Color.darkred)
